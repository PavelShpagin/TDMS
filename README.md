...
- На кожній ітерації worker читає локальний JSON з диска і завантажує у Google Drive.
- "Unsync" видаляє токен — наступний запуск побачить відсутність токена і припинить цикл.

**Технічна Архітектура:**

_Структура Celery Worker:_

- 1 процес worker з 2 внутрішніми потоками:
  - **Потік 1 (виконання):** блокується на `BLPOP celery 1` — подієво-орієнтований (не опитування), розблоковується миттєво при надходженні задачі
  - **Потік 2 (планувальник):** перевіряє відкладену чергу кожну ~1 секунду, переміщує готові задачі з `celery-delayed` → `celery`

_Черги Redis:_

- `celery` (LIST) — готові до виконання задачі
- `celery-delayed` (SORTED SET) — відкладені задачі з міткою часу ETA
- Сховище стану:
  - `tdms:sync:token:<DB>` — токен активної сесії синхронізації (видаляється при Unsync)
  - `tdms:sync:lock:<DB>` — короткочасний замок від паралельних запусків (TTL 10 сек)
  - `tdms:google:access_token` — кешований OAuth токен

_Потік даних:_

1. FastAPI/Worker → `ZADD celery-delayed {task}` (score=now+5s)
2. Потік-планувальник (кожну секунду) → `ZRANGEBYSCORE celery-delayed 0 now` → `LPUSH celery {task}` → `ZREM celery-delayed {task}`
3. Потік-виконувач (блокований на `BLPOP`) → розблоковується миттєво → виконує `sync_loop()` → завантажує в Drive → самоперепланування
